            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                if (results != null && results.count > 0) {
                    ...
                    for (int i = 0; i < mResultList.size(); i++) {
                        ...
                        bean.setmArea("" + item.getPrimaryText(STYLE_BOLD));
                        bean.setmLocationId("" + item.getPlaceId());
                        bean.setmLatitude("0");
                        bean.setmLongitude("0");
                        try {
                            bean.setmAddress("" + item.getSecondaryText(STYLE_BOLD));
                        } catch (Exception e) {
                            bean.setmAddress("");
                        }
